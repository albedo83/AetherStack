...
| Debayer method | Provide versioned methods with CFA-phase, edge, color, and artifact tests. | Strict deterministic `f64` Malvar-He-Cutler oracle and bounded transactional FITS execution implemented for all standard Bayer phases, with exact-sample, coefficient, border, overshoot, mask, band-invariance, checksum, cancellation, and memory-bound tests; Light-plan orchestration, RGB Blink presentation, representative-camera differential validation, and additional selectable methods remain |
