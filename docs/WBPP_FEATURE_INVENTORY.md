...
| Debayer method | Provide versioned methods with CFA-phase, edge, color, and artifact tests. | Strict deterministic `f64` Malvar-He-Cutler oracle, bounded FITS execution, and all-or-nothing reviewed-Light-plan RGB export implemented for all standard Bayer phases, with exact-sample, coefficient, border, overshoot, mask, band-invariance, provenance, checksum, cancellation, mutation, rollback, and memory-bound tests; RGB Blink presentation, representative-camera differential validation, and additional selectable methods remain |
...
| Image registration | Provide robust matching, transforms, interpolation, and residual diagnostics. | Pixel coordinates, affine composition/inversion, compensated residual summaries, and inspectable automatic-reference ranking implemented; matching, estimation, resampling, and corpus validation planned |
